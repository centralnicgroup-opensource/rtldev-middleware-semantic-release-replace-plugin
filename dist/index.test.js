...
test("prepare should replace using regex", async () => {
    ...
    await assertFileContentsContain("__init__.py", `__VERSION__ = "${context.nextRelease?.version}"`);
    await assertFileContents("build.gradle", `version = '${context.nextRelease?.version}'`);
});
    ...
    const replacements = [
        {
            ...
            from: ((_, context) => new RegExp(context?.lastRelease?.version || "", "g")),
            to: "3.0.0",
        },
    ];
    await prepare({ replacements }, context);
...
test("prepare passes the `context` as the final function argument to `to` callbacks", async () => {
    ...
    await prepare({ replacements }, context);
    await assertFileContentsContain("foo.md", `npm i oof@${context.nextRelease?.version}`);
    await assertFileContentsContain("foo.md", "yarn add foo@1.0.0");
});
...
test("prepare accepts an array of `to` replacements", async () => {
    ...
    await prepare({ replacements }, context);
    await assertFileContentsContain("foo.md", `npm install foo@${context.nextRelease?.version}`);
    await assertFileContentsContain("foo.md", `yarn add foo@${context.nextRelease?.version}`);
});
test("should update whmcs.json version using regex with countMatches and validate fixture integrity", async () => {
    ...
    await assertFileContentsContain("modules/addons/cnicdnsmanager/whmcs.json", `"version": "${context.nextRelease?.version}"`);
});
